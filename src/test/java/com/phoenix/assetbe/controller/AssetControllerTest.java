...
public class AssetControllerTest {
    ...
    public void setUp() throws Exception {
        ...
        User u1 = User.builder().email("user1@gmail.com").status(Status.ACTIVE).role(Role.USER).password(passwordEncoder.encode("1234")).emailVerified(true).provider(SocialType.COMMON).build();
        User u2 = User.builder().email("user2@gamil.com").status(Status.ACTIVE).role(Role.USER).password(passwordEncoder.encode("1234")).emailVerified(true).provider(SocialType.COMMON).build();
        User u3 = User.builder().email("user3@gamil.com").status(Status.ACTIVE).role(Role.USER).password(passwordEncoder.encode("1234")).emailVerified(true).provider(SocialType.COMMON).build();
    ...
    public void get_asset_details_with_user() throws Exception {
        ...
        // when
        ResultActions resultActions = mockMvc.perform(RestDocumentationRequestBuilders
                .get("/assets/{id}",id).with(user("user1@gmail.com")));
        String responseBody = resultActions.andReturn().getResponse().getContentAsString();
        System.out.println("테스트 : " + responseBody);
